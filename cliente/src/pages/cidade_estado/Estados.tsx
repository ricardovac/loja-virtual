...
const Estados = () => {
  ...
  const handleSaveRowEdits: MaterialReactTableProps<Estado>["onEditingRowSave"] = async ({
    exitEditingMode,
    row,
    values,
  }) => {
  ...
  const handleDeletarEstado = useCallback(
    async (row: MRT_Row<Estado>) => {
      ...
      if (response.status == 200) {
        setTodosEstados(todosEstados.filter((p) => p.id !== row.getValue("id")));
      }
    },
    [todosEstados]
  );
  ...
  const getCommonEditTextFieldProps = useCallback(
    (cell: MRT_Cell<Estado>): MRT_ColumnDef<Estado>["muiTableBodyCellEditTextFieldProps"] => {
      return {
          ...
          renderRowActions={({ row, table }) => (
            <Box sx={{ display: "flex", gap: "1rem" }}>
              ...
              <Tooltip arrow placement="right" title="Delete">
                <IconButton color="error" onClick={() => handleDeletarEstado(row)}>
                  <Delete />
                </IconButton>
              </Tooltip>
            </Box>
...
export const ModalCriarEstado = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
