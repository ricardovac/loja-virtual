...
const Cidades = () => {
  ...
  const handleSaveRowEdits: MaterialReactTableProps<Cidade>["onEditingRowSave"] = async ({
    exitEditingMode,
    row,
    values,
  }) => {
  ...
  const handleDeletarCidade = useCallback(
    async (row: MRT_Row<Cidade>) => {
      ...
      if (response.status == 200) {
        setTodasCidades(todasCidades.filter((p) => p.id !== row.getValue("id")));
      }
    },
    [todasCidades]
  );
  ...
  const getCommonEditTextFieldProps = useCallback(
    (cell: MRT_Cell<Cidade>): MRT_ColumnDef<Cidade>["muiTableBodyCellEditTextFieldProps"] => {
      return {
          ...
          renderRowActions={({ row, table }) => (
            <Box sx={{ display: "flex", gap: "1rem" }}>
              ...
              <Tooltip arrow placement="right" title="Delete">
                <IconButton color="error" onClick={() => handleDeletarCidade(row)}>
                  <Delete />
                </IconButton>
              </Tooltip>
            </Box>
...
export const ModalCriarCidade = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
...
const validateEstado = (sigla: string) => sigla.length >= 2 && sigla.length <= 20;
