...
const Pessoas = () => {
  ...
  const handleSaveRowEdits: MaterialReactTableProps<Pessoa>["onEditingRowSave"] = async ({
    exitEditingMode,
    row,
    values,
  }) => {
    if (!Object.keys(validationErrors).length) {
      const updatedPessoa: Pessoa = {
        ...
        cep: values.cep,
        permissaoPessoas: row.original.permissaoPessoas.map((permissaoPessoa: any) => ({
          permissao: {
  ...
  const handleDeletarPessoa = useCallback(
    async (row: MRT_Row<Pessoa>) => {
      ...
      if (response.status == 200) {
        setTodasPessoas(todasPessoas.filter((p) => p.id !== row.getValue("id")));
      }
    },
    [todasPessoas]
  );
  ...
  const getCommonEditTextFieldProps = useCallback(
    (cell: MRT_Cell<Pessoa>): MRT_ColumnDef<Pessoa>["muiTableBodyCellEditTextFieldProps"] => {
      return {
  ...
  const columns = useMemo<MRT_ColumnDef<Pessoa>[]>(
    () => [
      ...
      },
      {
        ...
        columns: [
          {
            accessorFn: (row) => `${row.permissaoPessoas.map((r: any) => r.permissao.nome)}`, //accessorFn used to join multiple data into a single cell
          ...
          renderRowActions={({ row, table }) => (
            <Box sx={{ display: "flex", gap: "1rem" }}>
              ...
              <Tooltip arrow placement="right" title="Delete">
                <IconButton color="error" onClick={() => handleDeletarPessoa(row)}>
                  <Delete />
                </IconButton>
              </Tooltip>
            </Box>
...
export const ModalCriarPessoa = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
  ...
  const handleChangePermissao = (event: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedPermissao((prevState) => ({
      ...prevState,
      [event.target.name]: event.target.checked ? Number(event.target.name) : null,
    }));
  };
          ...
          >
            {columns.map((column) =>
              column.accessorKey == "cidade.nome" ? (
                <FormControl fullWidth key={column.accessorKey}>
                  ...
                  >
                    {todasCidades.map((cidade) => (
                      <MenuItem value={cidade.id} key={cidade.id}>
                        {cidade.nome}
                      </MenuItem>
                    ))}
                  </Select>
                ...
                <FormControl component="fieldset" variant="standard" key={column.accessorKey}>
                  ...
                  <FormGroup>
                    {todasPermissoes.map((permissoes) => (
                      <FormControlLabel
                        key={permissoes.id}
                        control={
                          <Switch onChange={handleChangePermissao} name={String(permissoes.id)} />
                        }
                ...
                column.accessorKey !== "id" && (
                  <TextField
                    ...
                    inputProps={column.accessorKey === "cep" ? { maxLength: 8 } : {}}
                    onChange={(e) =>
                      setValues({
                        ...values,
                        [e.target.name]: e.target.value,
