...
const Permissoes = () => {
    ...
    const handleSaveRowEdits: MaterialReactTableProps<Permissao>["onEditingRowSave"] =
    ...
    const handleDeletarPermissao = useCallback(
        async (row: MRT_Row<Permissao>) => {
            ...
            if (response.status == 200) {
                setTodasPermissoes(
                    todasPermissoes.filter((p) => p.id !== row.getValue("id"))
                );
            }
        },
        [todasPermissoes]
    ...
    const getCommonEditTextFieldProps = useCallback(
        (
            cell: MRT_Cell<Permissao>
        ): MRT_ColumnDef<Permissao>["muiTableBodyCellEditTextFieldProps"] => {
            return {
                    ...
                    renderRowActions={({ row, table }) => (
                        <Box sx={{ display: "flex", gap: "1rem" }}>
                            ...
                            <Tooltip arrow placement="right" title="Delete">
                                <IconButton
                                    color="error"
                                    onClick={() => handleDeletarPermissao(row)}
                                >
                                    <Delete />
                                </IconButton>
                            </Tooltip>
...
export const ModalCriarPermissao = ({
    open,
    columns,
    onClose,
    onSubmit,
...
const validatePermissaoNome = (nome: string) =>
